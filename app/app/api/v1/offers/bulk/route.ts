...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'approve':
        result = await offerService.bulkApproveOffers(ids);
        break;
      case 'reject':
        result = await offerService.bulkRejectOffers(ids, reason);
        break;
      case 'send':
        result = await offerService.bulkSendOffers(ids);
        break;
      case 'delete':
        result = await offerService.bulkDeleteOffers(ids);
        break;
      default:
